...
void main() {
    ...
    float float_voxel_dimension = float(voxel_dimension);

    voxel_position = vec4(imageLoad(voxel_position_texture, gl_VertexID.x).xyz, 1.0);
    // Voxel structure maps points from 0 to 1, transform the from -1 to 1
    voxel_position.xyz = (voxel_position.xyz / float_voxel_dimension) * 2.0 - vec3(1.0,1.0,1.0);

    // Move point to middle of voxel (instead of bottom right of voxel)
    float half_pixel = 1.0 / float_voxel_dimension;
