bool within_second_half(float min, float half_node_size, float coordinate_position) {
...
// half of the node.
bvec3 calculate_node_subsection(vec3 node_coordinates, float half_node_size, vec3 fragment_position) {
  bvec3 subsection;
...
vec3 update_node_coordinates(
  vec3 current_node_coordinates,
  bvec3 subsection,
  float current_half_node_size
) {
  vec3 ret = current_node_coordinates;
  if (subsection.x) {
