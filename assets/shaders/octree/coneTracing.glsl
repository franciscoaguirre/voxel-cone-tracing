...
// layout (location = 1) out vec4 directIlluminationOnly;
// layout (location = 2) out vec4 indirectIlluminationOnly;
// layout (location = 3) out vec4 ambientOcclusionOnly;
...
vec4 gatherIndirectLight(vec3 position, vec3 normal, vec3 tangent, float maxDistance, float coneAngle, bool useLighting);
...
void main() {
    ...
    maxDistance = 0.5;
    useLighting = true;
    vec4 indirectLight = gatherIndirectLight(position, normal, tangent, maxDistance, coneAngle, useLighting); // Diffuse
    // TODO: Also gather specular indirect light
    ...
    if (imageToShow == 0) {
        ...
        finalImage = vec4(indirectLight.rgb, 1.0);
    } else if (imageToShow == 3) {
        finalImage = color * vec4(visibility * (directLight + indirectLight.rgb), 1.0);
    }
}
